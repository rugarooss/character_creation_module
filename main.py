...
class Character:
    ...
    def defence(self):
        value_defense = DEFAULT_DEFENCE + randint(*self.RANGE_VALUE_DEFENСE)
        return (f'{self.name} блокировал {value_defense} урона')
...
def start_training(char_class: Character) -> str:
    ...
    commands = {'attack': (char_class, char_class.attack()),
                'defence': (char_class, char_class.defence()),
                'special': (char_class, char_class.special())}
    cmd: str = None
    while cmd != 'skip':
